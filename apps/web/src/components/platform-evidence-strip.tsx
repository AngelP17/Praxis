...
export function PlatformEvidenceStrip({ items }: { items: EvidenceItem[] }) {
  function typeIcon(type: EvidenceItem["type"]) {
    switch (type) {
      ...
      case "screenshot": return <ImageIcon className="h-3.5 w-3.5 text-violet-400" />;
      ...
      case "link": return <LinkIcon className="h-3.5 w-3.5 text-violet-400" />;
    }
  }
  ...
  function typeBorder(type: EvidenceItem["type"]) {
    switch (type) {
      ...
      case "screenshot": return "border-violet-500/15 bg-violet-500/[0.04]";
      case "metric": return "border-emerald-500/15 bg-emerald-500/[0.04]";
      case "document": return "border-zinc-700/40 bg-zinc-950/40";
      case "link": return "border-violet-500/15 bg-violet-500/[0.04]";
    }
  }
