...
import { normalizeRootCause, recommendationFor, sourceFor, type DataStatus } from "@/components/sentinel-v2/command-room/types";
      ...
      {!ticket ? (
        ...
        <>
          <div className="mt-4 rounded-xl border border-zinc-700/80 bg-zinc-950/70 p-4">
            <div className="mono-data text-xs text-amber-200">{ticket.ticketId}</div>
          ...
          <div className="mt-2.5 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3.5">
            <div className="text-[11px] uppercase tracking-[0.18em] text-amber-100">Recommended workflow</div>
            <p className="mt-2 text-sm leading-6 text-amber-50">{recommendationFor(ticket)}</p>
          </div>
