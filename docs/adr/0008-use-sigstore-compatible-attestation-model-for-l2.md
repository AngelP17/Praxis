...
## Decision
We implement a **Sigstore-compatible attestation model** for the highest conformance level (**L2**).
...
- When verifying at level L2, the verifier validates that the signature and canonical hash are recorded in the transparency log.
...
- Enforced at L2 verification in `PraxisProofVerifier` which verifies the presence and structure of the Sigstore attestation block.
