...
| **Deterministic Replay** | Verifiable, auditable decision chains for defense applications | Bit-deterministic proof generation with CI gate enforcement. Every L0 proof is independently re-verifiable; L1 signed proofs are supported when a valid signature is attached; L2 attestation fails closed until implemented. |
...
| **Proof Protocol** | Chain of custody for operational decisions | Published PPP v0.1 as an open protocol. SHA-256 canonical proof hash, optional Ed25519 L1 signatures, and fail-closed L2 attestation boundary. |
